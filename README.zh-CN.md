...
下表每一项都有同数据集实测支撑（ID 级口径：命中 = 检索切片 ∈ 标注 relevant_chunks；脚本 `eval/scripts/ab_recall.py` / `diagnose_recall.py`，零 LLM 成本）：

| 变体（top-8） | Precision | Recall | 结论 |
|---|---|---|---|
| 原问题单路 | 0.166 | 0.478 | 基线 |
| 原问题 + 精排 | 0.171 | **0.497** | reranker **+0.019** —— 保留 |
| 仅 LLM 改写当主查询 | 0.135 | 0.398 | 单独改写 **−0.080** —— 有害 |
| 改写 + 子查询变体（无精排） | 0.160 | 0.467 | 多查询扳回损失 |
| **原话当主查询 + 子查询 + 精排** | **0.171** | **0.497**（K12：**0.523** vs 0.520） | 已采纳——主查询保持用户原话 |
| **+ 兄弟切片扩展（生产）** | **0.226** | **0.647**（K12：**0.725**） | **单项最大提升（+0.20）**——命中切片的 ±1 邻居自动入候选 |
| + HyDE 变体单独 | 0.174 | 0.503（K12：0.533） | 单独收益微弱 |
| 兄弟扩展 + HyDE | 0.218 | 0.618（K12：0.700） | HyDE 挤占窗口——默认关闭 |
| 再加单文档配额 3 | 0.154 | 0.410 | 精排前配额 **−0.087** —— 已移除 |
| 改写 + 子查询 + 精排（旧生产） | 0.171 | **0.497** | 原组合，已被上行取代 |

**BM25 vs learned-sparse 混合检索**（`bm25_compare.py`，jieba 分词 BM25，同语料 ID 级口径）：

| 方案 | Precision@8 | Recall@8 | Recall@12 |
|---|---|---|---|
| BM25 | 0.138 | 0.405 | 0.422 |
| BM25 + 精排 | 0.138 | 0.405 | 0.422 |
| **bge-m3 混合检索 + 精排（生产）** | **0.171** | **0.493** | **0.523** |

在本中文语料上，bge-m3 的 learned sparse 比经典 BM25 召回**高 0.09~0.10**（两个窗口一致）——这就是不选 BM25 的实测依据（零额外基础设施、学出来的 token 权重、无中文分词依赖）。

两个驱动设计的结构性发现：

- **37.5% 的标注切片根本不出现在 dense top-50**（纯向量语义盲区，典型如代码细节题）——这是多查询变体存在的原因；而**兄弟切片扩展**（命中的 ±1 邻居自动入候选）最终把 ID 召回从 **0.478 抬到 0.725**，是全程最大单项。
- 评估检索层跑的是**与生产完全一致的路径**（Planner → 多查询 → 兄弟扩展 → 精排，窗口 = `MAX_GRADE_SOURCES`），数字反映用户真实所得，而非简化代理。
